...
**Windows:**
Download `cagent-windows-amd64.exe` from the releases page and add to your PATH.
...
docker sandbox run cagent run container-nerdy-central.yaml
```
...
```bash
docker sandbox run cagent run container-nerdy-central.yaml "Tell me a joke about Kubernetes"
```
...
```bash
source .env
./cagent run container-nerdy-central.yaml "Two container experts walk into a bar..."
```
...
docker sandbox run cagent run comedy-central.yaml
```
...
docker sandbox run cagent run untrusted-agent.yaml

# Risky - Direct host access
...
```bash
cd specific-project
docker sandbox run cagent run agent.yaml
...
```bash
docker sandbox run cagent run container-nerdy-central.yaml --debug
```
...
3. Run in a sandbox:
```bash
docker sandbox run cagent run my-agent.yaml
```
